...
import ProgrammableTokens.OffChain.Env (
    DirectoryScriptRoot (..),
...
transferTokens assetName quantity target redeemer = do
    directoryNodes <- Query.registryNodes @era
    paramsNode <- Query.globalParamsNode @era
    ...
    (tx, _) <- Env.balanceTxEnv_ $ do
        BuildTx.transferProgrammableToken paramsNode inputs (transPolicyId policyId) directoryNodes
