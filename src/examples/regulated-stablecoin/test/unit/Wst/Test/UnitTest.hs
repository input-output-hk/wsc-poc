...
import ProgrammableTokens.OffChain.UTxODat (UTxODat (uDatum))
...
deployAll :: (MonadReader ScriptTarget m, MonadUtxoQuery m, MonadMockchain C.ConwayEra m, MonadFail m) => m ()
deployAll = do
    target <- ask
...
blacklistTransfer policy = failOnError @_ @(AppError C.ConwayEra) $ Env.withEnv $ do
    ...
    asAdmin @C.ConwayEra $
        Env.withDirectoryFor scriptRoot $
            Env.withTransferFromOperator @C.ConwayEra $
                Endpoints.deployBlacklistTx
                    >>= void . sendTx . signTxOperator admin
    ...
    asAdmin @C.ConwayEra $
        Env.withDirectoryFor scriptRoot $
            Env.withTransferFromOperator @C.ConwayEra $
                Endpoints.insertBlacklistNodeTx "" userPaymentCred
                    >>= void . sendTx . signTxOperator admin

    asWallet @C.ConwayEra Wallet.w2 $
        Env.withDirectoryFor scriptRoot $
            Env.withBlacklist ble $
                Env.withTransfer transferLogic $
                    Endpoints.transferSmartTokensTx policy aid 30 (C.PaymentCredentialByKey opPkh)
...
assertRegisteredTokensStayInMiniLedger ::
    forall era env m.
    ( MonadReader env m
    ...
    m ()
assertRegisteredTokensStayInMiniLedger = do
    registeredPolicyIds <-
        mapMaybe (either (const Nothing) Just . unTransPolicyId . key . uDatum)
            <$> ProgQuery.registryNodes @era
    baseCred <-
        asks
            ( C.PaymentCredentialByScript
                . C.hashScript
                . C.PlutusScript C.PlutusScriptV3
                . Env.dsProgrammableLogicBaseScript
                . Env.directoryEnv
            )
    ledgerUtxo' <- getUtxo
    let C.UTxO ledgerUtxo = fromLedgerUTxO C.shelleyBasedEra ledgerUtxo'
    let escapingOutputs =
            [ show txIn <> " holds registered policy " <> show policyId <> " outside programmableLogicBase"
            | (txIn, txOut) <- Map.toList ledgerUtxo
            , policyId <- registeredPolicyIds
            , ProgQuery.hasPolicyId policyId (ProgQuery.extractValue txOut)
            , paymentCredentialFromTxOut txOut /= baseCred
            ]
    when (not (null escapingOutputs)) $
        fail ("Registered programmable token escaped the mini-ledger:\n" <> unlines escapingOutputs)
...
paymentCredentialFromTxOut (C.TxOut (C.AddressInEra _ addr) _ _ _) =
    case addr of
        ...
        C.ByronAddress _ -> error "unexpected Byron address in programmable token test"
