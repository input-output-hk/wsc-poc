...
transferSmartTokens paramsTxIn blacklistNodes directoryNodes spendingUserOutputs (assetId, q) destinationCred = Utils.inBabbage @era $ do
    nid <- queryNetworkId
...
multiSeizeSmartTokens reason paramsTxIn toSeizePolicyId seizingTxos destinationCred directoryList = Utils.inBabbage @era $ do
    ...
    let seizedValue =
            C.policyAssetsToValue toSeizePolicyId $
                foldl
                    ( \acc (uOut -> utxoDat) ->
                        let filteredAssets =
                                foldMap
...
data FindProofResult era
    ...
    | -- | A node containing exactly the credential was found. (Negative result, transfer not OK)
      CredentialBlacklisted (UTxODat era BlacklistNode)
    | -- | A node was found that spans the credential but does not match it exactly. (Positive result, transfer OK)
      CredentialNotBlacklisted (UTxODat era BlacklistNode)
