...
instance FromHttpApiData PolicyIdHex where
    parseUrlPiece txt =
        case C.deserialiseFromRawBytesHex (TextEncoding.encodeUtf8 txt) of
...
data UserBalanceResponse
    ...
    , ubrUserLovelace :: C.Quantity
    -- ^ Total Ada locked by the user's public key hash (that is, value NOT in programmable token outputs)
    , ubrAdaOnlyOutputs :: Int
    -- ^ Count of ada-only outputs locked by user's public key hash
    }
    deriving stock (Eq, Show, Generic)
