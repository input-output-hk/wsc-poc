...
import Blammo.Logging.Simple (HasLogger, Message ((:#)), MonadLogger, logDebug,
...
import Wst.Server.PolicyIssuerStore (HasPolicyIssuerStore, addPolicyIssuerStore,
...
deploy config = do
  ...
  (tx, root) <- liftIO (runWstApp env' $ do
          Endpoints.deployCip143RegistryTx Production) >>= liftEither
